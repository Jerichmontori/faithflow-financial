...
export function LaporanKolomContent({ isPelsusView = false }: { isPelsusView?: boolean }) {
      ...
      <div className="panel no-print mb-5 grid gap-3 p-4 bg-muted/20 border md:grid-cols-2 xl:grid-cols-5">
        <div className="space-y-1.5 xl:col-span-2">
          ...
          <Popover open={openBudget} onOpenChange={setOpenBudget}>
            ...
            <PopoverContent className="w-[360px] sm:w-[440px] p-0" align="start">
              <div className="flex items-center border-b px-3 py-2">
                <Search className="mr-2 h-4 w-4 shrink-0 opacity-50" />
                <input
                  placeholder="Ketik kode atau nama pos anggaran…"
                  value={searchBudget}
              ...
              <div className="max-h-[300px] overflow-y-auto p-1 divide-y divide-border/20">
                <button
                  ...
                  className={cn(
                    "flex w-full items-center gap-2 rounded-sm px-2 py-2 text-left text-xs transition-colors hover:bg-accent hover:text-accent-foreground",
                    budgetId === "semua" ? "bg-primary/10 font-bold text-primary" : "font-medium",
                  )}
                >
                  <Check
                    className={cn(
                      "size-3.5 shrink-0",
                      budgetId === "semua" ? "opacity-100 text-primary" : "opacity-0",
                    )}
                  />
                  <span>Semua mata anggaran</span>
                </button>

                {filteredBudgetOptions.map((b) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full items-start gap-2 rounded-sm px-2 py-2 text-left text-xs transition-colors hover:bg-accent hover:text-accent-foreground",
